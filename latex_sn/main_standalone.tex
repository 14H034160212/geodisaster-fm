...
\cite{brown2024alphaearth,jakubik2023prithvi,xiong2024dofa}. The implicit assumption is that the learned representation
...
\textbf{Active learning} \cite{sener2018coreset,gal2017dropout} asks ``how do we get the most
...
\subsection{Reading guide --- how the Results sections map onto H1 vs H2}\label{reading-guide-how-the-results-sections-map-onto-h1-vs-h2}
...
R3 & \textbf{Test of H2(b) --- Does $\tau$-recalibration recover F1?} & 15/16 measured event-optimal $\tau$ $\neq$ 0.5 across 3 benchmarks(flood/damage/wildfire);up to +0.235 F1 recovered;magnitude is hazard-specific \\
...
\subsection{R1 --- The dispatcher: real-event answers, in seconds}\label{r1-the-dispatcher-real-event-answers-in-seconds}
...
drift); (b) Test of H2(a) --- F1@$\tau$* vs F1@0.5 scatter across all 10
events (every point above y = x, ranking survives, Pakistan +0.184 F1
from $\tau$ alone); (c) Test of H2(b) --- recalibration recovers F1 on every
...
\subsection{R2 --- Test of H2(a): Does the pixel ranking transfer across events? --- Generalisation across regions and hazards}\label{r2-test-of-h2a-does-the-pixel-ranking-transfer-across-events-generalisation-across-regions-and-hazards}
...
$\neq$ 0.5} but on the opposite side of the default (Fig. 4b).
...
\tightlist
\item
  \textbf{Generalised Advantage Estimation (GAE-$\lambda$ = 0.95)} in place of raw
...
Fig. 5e for the v1$\leftrightarrow$v2 comparison): the v1 PPO was an
...
\emph{{[}Fig. 2 --- source \texttt{outputs/figures/fig26\_loeo\_v1\_v2.png} (LOEO-v1$\leftrightarrow$v2 +
...
\subsubsection{R4c --- The zero-label test: cross-disaster calibration drift is NOT pure label shift, so the four labels are necessary}\label{r4c-the-zero-label-test-cross-disaster-calibration-drift-is-not-pure-label-shift-so-the-four-labels-are-necessary}
...
unlabelled target data \cite{saerens2002adjusting,lipton2018detecting}. We test the two
...
\subsection{R6 --- System-level metrics: time-to-answer and reproducibility}\label{r6-system-level-metrics-time-to-answer-and-reproducibility}
...
\subsection{Implication 1 --- for the disaster-response community}\label{implication-1-for-the-disaster-response-community}
...
\subsection{Implication 2 --- for foundation-model research}\label{implication-2-for-foundation-model-research}
...
\subsection{Implication 3 --- for the active-learning and post-hoc-calibration literatures}\label{implication-3-for-the-active-learning-and-post-hoc-calibration-literatures}
...
\emph{decision threshold $\tau$}. Empirically (Results 1) this is the dominant lever
...
LOEO 200-pair protocol our PPO recovers $\approx$ 99 \% of its F1 (R4:
...
\begin{figure}[t]
  ...
  (large for floods and damage, small for wildfires), but 15 of 18
  event-optimal thresholds differ from the 0.5 default.}
