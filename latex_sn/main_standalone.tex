...
mitigation work treats it as a \textbf{representation problem}: scale training data
across events \cite{yadav2022sarflood,misra2025globalfloods},
extend to harder hazard settings \cite{hu2023burnseverity,aldabbagh2023wildfire},
or increase backbone pre-training and modality breadth
\cite{brown2024alphaearth,jakubik2023prithvi,xiong2024dofa,wu2025skysense}.
...
segmentation work \cite{gong2024crossearth}. In disaster mapping
specifically, the closest out-of-domain precedent already shows that
in-distribution performance is a poor predictor of cross-event
performance \cite{bensonecker2020outofdomain}.
...
model (H1) and recalibrating its operating point (H2). A related
Nature Communications precedent applies the same budget-constrained
querying logic to active label cleaning (data quality), not operating-
point calibration \cite{bernhardt2022activelabel}.
\textbf{Post-hoc
...
\% of road-kilometres are flagged as affected). We deliberately stop
short of claiming dollar or casualty effects, which require controlled
...
which our results bound at four chips. These deployment implications align with two recent
Nature Communications results: one argues for integrating
Earth-observation and meteorological foundation models into multi-hazard
early-warning systems \cite{reichstein2025earlywarning}, and another
demonstrates that data-driven machine learning is already operational
for global fire-activity prediction \cite{digiuseppe2025fireactivity}.
...
\emph{matched inputs} is small. Where foundation models add
...
a recent Nature Machine Intelligence editorial has flagged that
geospatial foundation models do not automatically resolve robustness
challenges in cross-event generalisation or real-time monitoring
\cite{nmi2025editorial} --- the
...
(Supplementary Information), and we expect active-selection
