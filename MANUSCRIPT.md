...
single threshold restores most lost performance. Across 18 events on
...
This four-label fix reduces per-event human labeling from days to minutes.
...
sharply on a new event. This degradation is the central operational
obstacle to scalable machine-learnt disaster mapping, yet most
mitigation work treats it as a **representation problem**: scale training data across
events [Yadav 2022; Misra 2025], extend to harder hazard settings
[Hu 2023; Al-Dabbagh 2023], or increase backbone pre-training and modality
breadth [Brown 2024; Jakubik 2023 (Prithvi); Xiong 2024 (DOFA); Wu 2025].
The underlying assumption is that representation transfer fails and must
be repaired with a better representation, a framing explicit in domain-generalisable remote-sensing
segmentation work [Gong 2024]. In disaster mapping specifically, the
closest out-of-domain precedent already shows that in-distribution
performance is a poor predictor of cross-event performance
[Benson & Ecker 2020].
...
model (H1) and recalibrating its operating point (H2). A related Nature
Communications precedent applies the same budget-constrained querying
logic to active label cleaning (data quality), not operating-point
calibration [Bernhardt 2022].
**Post-hoc
...
We discriminate H1 from H2 with **four independent lines of evidence** —
...
Copernicus EMS Rapid Mapping cycle — and delivers the auditable,
...
mapping well inside the first 72-hour window when evacuation, search
prioritisation and aid-allocation decisions are most time-sensitive.
...
the many significant events worldwide each year.

These savings are operational, not abstract: the calibrated agent's
flooded-area totals track analyst hand-labels at r = 0.971 in the
event-ranking regime responders use, and directly feed exposure figures
that size response (on the USA test event, 2.4 % of buildings and 6.6 %
of road-kilometres are flagged as affected). We deliberately stop short
of claiming dollar or casualty effects, which require controlled
deployment trials; our claim is a defensible cost-, time- and
feasibility-reduction argument anchored in the label-budget ratio,
published EMS service targets and measured machine time. The practical
limit on this pipeline is not compute but the human labelling step,
which our results bound at four chips. These deployment implications
align with two recent Nature Communications results: one argues for
integrating Earth-observation and meteorological foundation models into
multi-hazard early-warning systems [Reichstein 2025], and another
demonstrates that data-driven machine learning is already operational
for global fire-activity prediction [Di Giuseppe 2025].
...
*matched inputs* is small. Where foundation models add
...
its result — comparable F1, comparable calibration headroom,
...
Nature Machine Intelligence editorial has flagged that geospatial
foundation models do not automatically resolve robustness challenges in
cross-event generalisation or real-time monitoring
[Nature Machine Intelligence Editorial 2025] — the
...
one-parameter post-hoc adjustment. We give one instantiation
...
(Supplementary Information), and we expect active-selection heuristics
targeting the decision boundary to perform comparably on this task.

For the **label-shift literature** specifically, our zero-label test
(see ‘The four labels are necessary’) adds a cautionary result: in a realistic cross-event
