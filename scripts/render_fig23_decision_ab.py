...
un = json.loads(Path("outputs/layer3_ppo/decision_ab_unet.json").read_text())
ae = json.loads(Path("outputs/layer3_ppo/decision_ab_ae.json").read_text())
...
ax1.set_title("(a) Pixel F1 — pixel-reward best, decision-reward\n"
              "sacrifices ~2–3 pp (paired-significant, p≤0.009)",
...
ax2.set_title("(b) Decision area error — decision-reward wins,\n"
              "AlphaEarth −62 % (effect size large, n.s. at n=10×4)",
              fontsize=10.5, loc='left')
ax2.grid(True, alpha=0.3, axis='y')
ax2.set_ylim(0, max(get(un, "base", "dec_err")["ci95"][1], 18) * 1.05)

fig.suptitle("Reward alignment is a control knob: pixel-F1 vs decision-area reward (10-seed paired A/B)",
